...
public class AuditLogController {
        ...
        @Operation(summary = "Create batch of audit logs", description = """
                        Creates multiple immutable audit logs in a single request.

                        Authentication is performed using the X-API-Key header.

                        Each audit log in the batch is validated and processed individually.
                        """)
        @ApiResponses({
                        @ApiResponse(responseCode = "201", description = "Audit logs created successfully"),
                        @ApiResponse(responseCode = "401", description = "Invalid API Key", content = @Content),
                        @ApiResponse(responseCode = "422", description = "Invalid request", content = @Content),
                        @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
        })
        @PostMapping("/batch")
        public ResponseEntity<Void> postBatch(
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "The list of audit logs to create") @Valid @RequestBody List<AuditLogRequestDTO> auditLogRequestDTOs,
                        @Parameter(description = "The API key for authentication", example = "your-api-key", in = ParameterIn.HEADER) @RequestHeader("X-API-Key") String apiKey) {
                auditLogUseCase.createAll(apiKey, auditLogRequestDTOs.stream()
                                .map(auditLogWebMapper::toDomain)
                                .toList());
